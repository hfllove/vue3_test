  ...
  <div>
    ...
    <el-table
      ...
      <el-table-column label="操作" width="300" align="center">
        <template slot-scope="{row}">
          <HitnButton size="mini" type="info" icon="el-icon-info" title="分配权限"
            @click="$router.push(`/acl/role/auth/${row.id}?roleName=${row.roleName}`)"/>

          <HitnButton size="mini" type="primary" icon="el-icon-check" title="确定" 
            @click="updateRole(row)" v-if="row.edit"/>
          <HitnButton size="mini" type="primary" icon="el-icon-edit" title="修改角色" 
            @click="row.edit= true" v-if="!row.edit"/>

            
          <HitnButton size="mini" type="danger" icon="el-icon-delete" title="删除角色"
